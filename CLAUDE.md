...
This repo is a fork of `mattpocock/skills`. Before changing anything outside a `SKILL.md`, read [.agents/fork.md](./.agents/fork.md): it sets out which files diverge on purpose, how to sync from upstream, how to propose a change back, and where a new skill goes. In particular, skills that are ours alone live in their own bucket and get no `docs/` page. Currently that is `product-management/` (`grill-bet` and `bet-writing`); it ships in the plugin and has a flat-list bucket `README.md`, and is otherwise outside the promoted-bucket rules above.
